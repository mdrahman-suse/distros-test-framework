...
resource "aws_instance" "master" {
  ...
  provisioner "file" {
    source      = "../scripts/rke2_master.sh"
    destination = "/tmp/rke2_master.sh"
  }
  provisioner "remote-exec" {
    inline = [<<-EOT
      chmod +x /tmp/rke2_master.sh
      sudo /tmp/rke2_master.sh ${var.node_os} ${local.fqdn} ${self.public_ip} ${self.private_ip} "${var.enable_ipv6 ? self.ipv6_addresses[0] : ""}" ${var.install_mode} ${var.install_version} "${var.install_channel}" "${var.install_method}" "${var.datastore_type}" "${data.template_file.test.rendered}" "${var.server_flags}" ${var.username} ${var.password}
    EOT
    ]
  }
