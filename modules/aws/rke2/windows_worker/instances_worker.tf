resource "aws_instance" "windows_worker" {
  ...
  user_data              = templatefile("../scripts/join_rke2_windows_agent.tftpl", {serverIP: "${local.master_ip}", token: "${local.node_token}", install_mode: "${var.install_mode}", install_version: "${var.install_version}"})
