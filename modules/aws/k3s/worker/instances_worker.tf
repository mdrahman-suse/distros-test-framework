resource "aws_instance" "worker" {
  ...
  provisioner "file" {
    source               = "../scripts/join_k3s_agent.sh"
  ...
  provisioner "remote-exec" {
    inline = [<<-EOT
      ...
      sudo /tmp/join_k3s_agent.sh ${var.node_os} ${local.master_ip} ${local.node_token} ${self.public_ip} ${self.private_ip} "${var.enable_ipv6 ? self.ipv6_addresses[0] : ""}" ${var.install_mode} ${var.install_version} "${var.install_channel}" "${var.worker_flags}" ${var.username} ${var.password}
    EOT
    ]
  }
}
