...
resource "aws_instance" "master" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo /tmp/k3s_master.sh ${var.node_os} ${local.fqdn} ${self.public_ip} ${self.private_ip} \"${var.enable_ipv6 ? self.ipv6_addresses[0] : ""}\" ${var.install_mode} ${var.install_version} \"${var.install_channel}\" ${var.etcd_only_nodes} ${var.datastore_type} \"${data.template_file.test.rendered}\" \"${var.server_flags}\" ${var.username} ${var.password}"
    ]
  }
...
resource "aws_instance" "master2-ha" {
  ...
  provisioner "file" {
    source = "../scripts/join_k3s_master.sh"
  ...
  provisioner "remote-exec" {
    inline = [ <<-EOT
      ...
      sudo /tmp/join_k3s_master.sh ${var.node_os} ${local.fqdn} ${local.master_node_ip} ${local.node_token} ${self.public_ip} ${self.private_ip} "${var.enable_ipv6 ? self.ipv6_addresses[0] : ""}" ${var.install_mode} ${var.install_version} "${var.install_channel}" ${var.datastore_type} "${data.template_file.test.rendered}" "${var.server_flags}" ${var.username} ${var.password} > /tmp/join_k3s_master.log 2>&1
    EOT
    ]
  }
}
