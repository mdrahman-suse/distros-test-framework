...
func scpRke2Files(cluster *shared.Cluster, nodeType, ip string) error {
	joinLocalPath := shared.BasePath() + fmt.Sprintf("/modules/scripts/join_rke2_%s.sh", nodeType)
...
func k3sAgentSCP(cluster *shared.Cluster, ip string) error {
	...
	joinLocalPath := shared.BasePath() + fmt.Sprintf("/modules/scripts/join_k3s_%s.sh", agent)
...
func k3sServerSCP(cluster *shared.Cluster, ip string) error {
	...
	joinLocalPath := shared.BasePath() + fmt.Sprintf("/modules/scripts/join_k3s_%s.sh", master)
