...
# ----------------------------------------------------------------------------
# Elastic IP support (gated on var.create_eip).
#
# The rebootinstances test needs stable public IPs across stop/start cycles.
# Upstream's cluster_nodes module gives every instance an ephemeral public IP
# (associate_public_ip_address = true) which AWS replaces on reboot. We
# allocate one EIP per node and associate it, then re-emit the relevant
# outputs (kube_api_host, cluster_nodes_json) with the EIP-backed addresses
# so the rest of the framework (inventory generator, node extractor, suite
# tests) sees a single consistent set of IPs.
# ----------------------------------------------------------------------------

locals {
  # Replicate the name-generation logic from upstream's cluster_nodes module
  # (tofu/aws/modules/cluster_nodes/main.tf) so we know each instance's Name
  # tag at plan time, without depending on cluster_nodes_json being
  # computed first.
  ...
  # First node with "etcd" in its roles becomes the cluster-init / primary
  # master. Matches upstream's `first_etcd_index` exactly so the primary IP
  # we expose as kube_api_host is the same node upstream picks.
  first_etcd_index = index(
    [for n in local.temp_node_names : contains(n.role, "etcd")],
    true,
  )

  node_names = [
    for idx, n in local.temp_node_names :
    idx == local.first_etcd_index ? "master" : n.name
  ]

  primary_name  = local.first_etcd_index >= 0 ? "master" : ""
...
# inventory.go) and the node extractor (nodeextractor.go) can read it via
# `tofu output -raw cluster_nodes_json`. When create_eip is true, rewrite
# both the per-node public_ip and metadata.kube_api_host to reference EIPs
# so reboots don't change them.
